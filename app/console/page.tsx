...
export default function Demo() {
  ...
  async function attachAll() {
    setBusy("attach"); try { await call("/api/console/mvp1", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "attachAll" }) }); await loadSpaces(); } finally { setBusy(""); }
  }
  ...
  async function applyGoverned() { setBusy("apply"); try { await call("/api/console/mvp2", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "applyGoverned", spaceId, contentTypeId: denyCt, denyAction }) }); await loadSpace(spaceId); } finally { setBusy(""); } }
  async function removeGoverned() { setBusy("remove"); try { await call("/api/console/mvp2", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "removeGoverned", spaceId }) }); await loadSpace(spaceId); } finally { setBusy(""); } }
  async function addUser() { setBusy("add"); try { await call("/api/console/mvp2", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "addUser", spaceId, email, roleId: addRole }) }); setEmail(""); await loadSpace(spaceId); } finally { setBusy(""); } }
  async function removeUser(membershipId: string) { setBusy("rm" + membershipId); try { await call("/api/console/mvp2", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "removeUser", spaceId, membershipId }) }); await loadSpace(spaceId); } catch { /* err shown */ } finally { setBusy(""); } }
  async function applyAll() { setBusy("applyAll"); setBulkOut("Running across all spaces… (may take ~1 min)"); try { const d = await call("/api/console/mvp2", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "applyGovernedAll", contentTypeId: bulkCt, denyAction: bulkAction }) }); setBulkOut(JSON.stringify(d, null, 2)); if (spaceId) await loadSpace(spaceId); } catch { /* err shown */ } finally { setBusy(""); } }
